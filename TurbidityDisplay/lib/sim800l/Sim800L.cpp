 ...
 *  	en el archivo header, "Sim800L.h", por defecto los pines vjjjienen configurado en
 *  	RX=10 TX=11.
...
bool Sim800L::setSleepMode(bool state)
{
    ...
    if ( (_readSerial(1000).indexOf("OK")) == -1)
    {
        return true;
    }
    else return false;
...
bool Sim800L::checkAT()
{
  this->SoftwareSerial::print(F("AT\r"));
  if ( (_readSerial(1000).indexOf("OK")) == -1)
  {
      return true;
  }
...
bool Sim800L::checkGsmStatus()
{
  this->SoftwareSerial::print(F("AT+CREG?\r"));
  if ( (_readSerial(1000).indexOf("+CREG: 0,1")) == -1)
  {
      return true;
  }
  else return false;
...
bool Sim800L::checkGprsStatus()
{
  this->SoftwareSerial::print(F("AT+CGATT?\r"));
  if ( (_readSerial(1000).indexOf("+CGATT: 1")) == -1)
  {
      return true;
  }
  else return false;
...
bool Sim800L::setFunctionalityMode(uint8_t fun)
{
        ...
        {
        case 0:
            this->SoftwareSerial::print(F("AT+CFUN=0\r "));
            break;
        case 1:
            this->SoftwareSerial::print(F("AT+CFUN=1\r "));
            break;
        case 4:
            this->SoftwareSerial::print(F("AT+CFUN=4\r "));
            break;
        }
...
bool Sim800L::checkSimCardInfo()
{
  this->SoftwareSerial::print(F("AT+CPIN?\r"));
  if ((_readSerial(1000).indexOf("READY")) == -1) {
    return true;
  }
  else return false;
  // Error found, return 1
...
void Sim800L::reset()
{
    ...
    while (_readSerial(1000).indexOf("OK")==-1 )
    {
        this->SoftwareSerial::print(F("AT\r\n"));
    }
...
void Sim800L::activateBearerProfile()
{
    ...
    _buffer=_readSerial(1000);  // set bearer parameter
    this->SoftwareSerial::print (F(" AT+SAPBR=3,1,\"APN\",\"internet\" \r\n" ));
    _buffer=_readSerial(1000);  // set apn
    this->SoftwareSerial::print (F(" AT+SAPBR=1,1 \r\n"));
    delay(1200);
    _buffer=_readSerial(1000);			// activate bearer context
    this->SoftwareSerial::print (F(" AT+SAPBR=2,1\r\n "));
    delay(3000);
    _buffer=_readSerial(1000); 			// get context ip address
}
...
uint8_t Sim800L::unread(){
    String index;
    this->SoftwareSerial::print(F("AT+CMGF=1\r"));
    if (_readSerial(1000).indexOf("OK") == -1) {
        return 0;
    }
    this->SoftwareSerial::print(F("AT+CMGL=\"ALL\"\r"));
    _buffer = _readSerial(2000);
    index = _buffer.substring(_buffer.indexOf("+CMGL:")+7, _buffer.indexOf("+CMGL:")+8);
    return index.toInt();
}


String Sim800L::getNumberSms(String msg)
{
    if (msg.length() > 10) //avoid empty sms
    {
        return msg.substring(msg.indexOf("\",\"")+3, msg.indexOf("\",\"")+17);
    }
    else
    {
        return "none";
...
String Sim800L::readSms(uint8_t index)
{
    ...
    if (( _readSerial(5000).indexOf("ERROR")) == -1)
    {
        ...
        {
            return _buffer.substring(_buffer.indexOf("+CMGR:"));
        }
        else return "";
    }
    else
...
String Sim800L::getSmsContain(String msg)
{
  if (msg.length() > 10) {
...
String Sim800L::_readSerial(uint32_t timeout)
{
    ...
    while ((start+timeout) > millis()) {
      while (this->SoftwareSerial::available()) {
        char c = this->SoftwareSerial::read();
        str+=c;
      }
    }
    return str;
